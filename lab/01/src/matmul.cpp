...
static Config parse_args(int argc, char** argv) {
    if (argc < 4) {
        throw std::runtime_error("Usage: ./matmul_mpi <m> <n> <k> [--full-print] [--no-print]");
    }
...
static void compute_partition(int rows, int ranks, std::vector<int>& row_counts, std::vector<int>& displacements) {
    row_counts.assign(ranks, 0);
    displacements.assign(ranks, 0);
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::vector<int> a_counts(world_size, 0);
        std::vector<int> a_offsets(world_size, 0);
        std::vector<int> c_counts(world_size, 0);
        std::vector<int> c_offsets(world_size, 0);
        ...
        std::vector<double> full_b(b_size);
        ...
        std::vector<double> local_c(local_c_size);
        ...
        if (world_rank == 0) {
            if (!config.no_print) {
                print_matrix(full_a, config.m, config.n, "A", config.full_print);
                print_matrix(full_b, config.n, config.k, "B", config.full_print);
                print_matrix(full_c, config.m, config.k, "C", config.full_print);
            }

            const double flops = 2.0 * static_cast<double>(config.m) * config.n * config.k;
            const double gflops = flops / elapsed / 1e9;

            std::cout << "Version: MPI-v1\n";
